...
public class MatchedProteinsBuilder {
	 ...
	 * @throws Exception
	 */
	public Map<String, Integer> buildMatchedProteins( LimelightInput limelightInputRoot, File fastaFile, Collection<ModAReportedPeptide> reportedPeptides, String decoyPrefix ) throws Exception {
		
		System.err.print( " Matching peptides to proteins..." );

		// all protein names matched by any peptide
		Collection<String> proteinNamesInResults = getAllProteinsFromResults( reportedPeptides, decoyPrefix );

		// find the proteins matched by any of these peptides (map of sequence => fasta annotations
		Map<String, MatchedProteinInformation> proteins = getProteinsUsingProteinNames( proteinNamesInResults, fastaFile );

//		{
//			Collection<String> proteinNamesNotFoundInFasta = getProteinNamesNotFoundInFasta(proteinNamesInResults, proteins);
//			if (proteinNamesNotFoundInFasta.size() > 0) {
//
//				for(String p : proteinNamesNotFoundInFasta) {
//					proteinNamesInResults.remove(p);
//				}
//			}
//		}

		// map and validate protein names to protein sequence ids
		Map<String, Integer> proteinNameIdMap = getMatchedProteinIdsForProteinNames( proteins, proteinNamesInResults );
	 ...
	 * @return
	 */
	private Collection<String> getAllProteinsFromResults( Collection<ModAReportedPeptide> reportedPeptides, String decoyPrefix ) {

		Collection<String> proteinNames = new HashSet<>();

		for( ModAReportedPeptide reportedPeptide : reportedPeptides ) {
			for(String n : reportedPeptide.getProteinMatches()) {
				if(decoyPrefix == null || !n.startsWith(decoyPrefix)) {
					proteinNames.add(n);
				}
			}
		}
		...
		for( String proteinName : proteinNames ) {
			...
			for( MatchedProteinInformation mpi : proteinSequenceAnnotations.values() ) {
				...
				for( FastaProteinAnnotation fpa : mpi.getFastaProteinAnnotations() ) {
					...
					// in MODa, it seems to truncate protein names, so that sp|P02768|ALBU_HUMAN becomes sp|P02768
					// what would possess a human being to do this??
					if( fpa.getName().startsWith( proteinName + "|" ) || fpa.getName().equals(proteinName) ) {
	...
	private boolean fastaEntryContainProteinName( String proteinName, FASTAEntry fastaEntry ) {
		...
		for( FASTAHeader header : fastaEntry.getHeaders() ) {
			...
			if( header.getName().startsWith( proteinName + "|" ) || header.getName().equals( proteinName ) ) {
				return true;
			}
	 ...
	 *
	 * @param proteinNamesFoundInResults The collection of distinct protein names reported by Comet as a match for any peptide
	 * @param fastaFile The FASTA file that was searched.
	 * @return
	 * @throws Exception if there is a problem reading the FASTA file
	 */
	private Map<String, MatchedProteinInformation> getProteinsUsingProteinNames( Collection<String> proteinNamesFoundInResults, File fastaFile ) throws Exception {
		...
		try ( FASTAFileParser parser = FASTAFileParserFactory.getInstance().getFASTAFileParser( fastaFile ) ) {
			...
			for ( FASTAEntry entry = parser.getNextEntry(); entry != null; entry = parser.getNextEntry() ) {
				...
				for( String proteinName : proteinNamesFoundInResults ) {
